 ...
 *
 * Origine : table @VA 0x971010 du binaire DEVMEL AirSendWebService (chemin PFX @0x7fa2c0).
 * La cle KEELOQ (crypt_key) n'est PAS aleatoire ni derivee par-serial : elle est
 * SELECTIONNEE dans une table de 15 cles par un index encode dans le serial :
 *     (serial & 0x3FF) == 0x067      // marqueur famille PFX/Profalux
 *     index = (serial >> 10) - 1     // dans [0,14]
 ...
 * @param out_idx  recoit l'index [0,14] (peut etre NULL)
